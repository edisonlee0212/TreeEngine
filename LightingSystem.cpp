...
LightingSystem::LightingSystem() : SystemBase() {
	...
	size_t size = 4 + Default::ShaderIncludes::MaxDirectionalLightAmount * sizeof(DirectionalLight);
	...
	size = 4 + Default::ShaderIncludes::MaxPointLightAmount * sizeof(PointLight);
	...
	size = 4 + Default::ShaderIncludes::MaxSpotLightAmount * sizeof(SpotLight);
	...
	dl.direction = glm::vec4(-0.2f, -1.0f, -0.3f, 0.0f);
	dl.ambient = glm::vec4(0.05f);
...
void LightingSystem::Update() {
	...
	if (size != 0)glBufferSubData(GL_UNIFORM_BUFFER, 4, size, &_DirectionalLights[0]);
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
	...
	if (size != 0)glBufferSubData(GL_UNIFORM_BUFFER, 4, size, &_PointLights[0]);
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
	...
	if (size != 0)glBufferSubData(GL_UNIFORM_BUFFER, 4, size, &_SpotLights[0]);
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
}
